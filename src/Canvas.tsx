...
const Canvas = () => {
  const { canvas, username, selectedColor, updatePixel, cooldown, setIsShaking } = useStore();
  const [tempPixel, setTempPixel] = useState<{ x: number; y: number; color: string } | null>(null);
  const [fadingPixel, setFadingPixel] = useState<{ x: number; y: number; color: string } | null>(null);
    ...
    >
      ...
      >
        {Array.from({ length: CANVAS_SIZE * CANVAS_SIZE }).map((_, index) => {
          ...
          const tempPixelColor = tempPixel && tempPixel.x === x && tempPixel.y === y ? tempPixel.color : null;
          const fadingPixelColor = fadingPixel && fadingPixel.x === x && fadingPixel.y === y ? fadingPixel.color : null;

          return (
            <div
              key={index}
              className={`w-full h-full border border-gray-300 ${fadingPixel && fadingPixel.x === x && fadingPixel.y === y ? 'transition-all duration-1000' : ''}`}
              style={{ backgroundColor: tempPixelColor || fadingPixelColor || (pixel ? pixel.color : '#FFFFFF') }}
